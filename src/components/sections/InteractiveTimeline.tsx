...
export default function InteractiveTimeline() {
  ...
  return (
    <section id="section-timeline" className="py-20 px-6" dir={dir}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative" key={animationResetKey}>
          ...
          {/* Era nodes */}
          <div className={`grid grid-cols-2 md:grid-cols-5 gap-4 md:gap-6 relative z-10 ${isAr ? 'direction-rtl' : ''}`}>
            {timelineEras.map((era, index) => {
              ...
              return (
                <motion.button
                  ...
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: 0.2 + index * 0.15, duration: 0.5, ease: [0.4, 0, 0.2, 1] }}
                  onClick={() => setSelectedEra(isSelected ? null : era.id)}
                  className={`flex flex-col items-center text-center group cursor-pointer touch-target p-3 rounded-2xl transition-all duration-300
                    ${isSelected
                ...
                >
                  <motion.div
                    className={`w-[52px] h-[52px] rounded-full flex items-center justify-center text-2xl mb-3
        ...
        <motion.div
          ...
          className="mt-12 key-fact-card rounded-2xl p-5 max-w-3xl mx-auto"
        >
          <div className={`flex items-start gap-3 ${isAr ? 'flex-row-reverse' : ''}`}>
            <span className="text-2xl">💡</span>
            <div className={isAr ? 'text-right' : ''}>
              <h4 className="font-semibold text-primary-800 text-sm mb-1">{t('keyFactLabel', 'حقيقة مهمة')}</h4>
